...
export default function ReportsScreen() {
  ...
  const fetchReports = async () => {
    try {
      setLoading(true);
      // CORRECCIÓN: Volvemos a la relación limpia y estructurada con 'center_id' para evitar crasheos PGRST200
      const { data, error } = await supabase
        .from('reports')
        .select('id, reason, description, status, created_at, reporter_id, center:center_id(id, name)')
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              // 2. Si es suspender, bajamos el centro a nivel de estado
              ...
              Alert.alert('Error', 'No se pudo completar la operación en la base de datos.');
            }
          }
        }
      ]
  ...
  const renderItem = ({ item }: { item: any }) => (
    <View style={styles.card}>
      ...
      <Text style={styles.reporterName}>Reportado por Usuario ID: {item.reporter_id ? item.reporter_id.substring(0, 8) : 'Anónimo'}</Text>
      
      <View style={styles.descriptionBox}>
        <Text style={{ fontWeight: '700', color: AuraColors.textPrimary, marginBottom: 2 }}>Motivo: {item.reason || 'No especificado'}</Text>
